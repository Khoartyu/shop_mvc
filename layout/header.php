        ...
        <div class="container">
            ...
            <div class="d-flex d-md-none align-items-center">
                ...
                <a href="/shop_mvc/baocao/giohang.php" class="nav-link text-white px-2 position-relative">
                    ...
                    <span class="position-absolute top-30 start-70 translate-middle badge rounded-pill bg-danger" id="cart-total-count-mobile" style="font-size: 0.6rem;">0</span>
                </a>

                <?php if (isset($_SESSION['customer_name'])): ?>
                    <a href="#" onclick="handleLogout(event)" class="nav-link text-danger px-2" title="Đăng xuất">
                ...
                <ul class="navbar-nav d-none d-md-flex align-items-center">
                    ...
                    <li class="nav-item">
                        <a href="/shop_mvc/baocao/giohang.php" class="nav-link position-relative">
                            <i class="fas fa-shopping-cart"></i>
                            <span class="position-absolute top-30 start-70 translate-middle badge rounded-pill bg-danger" id="cart-total-count" style="display: none;">0</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <?php if (isset($_SESSION['customer_name'])): ?>
                            <div class="d-flex align-items-center ms-2">
                                <span class="nav-link text-warning fw-bold" style="font-size: 0.9rem; cursor: default;">
                                    Hi, <?= htmlspecialchars($_SESSION['customer_name']) ?>
                                </span>
                                <a class="nav-link text-danger ms-1" href="#" onclick="handleLogout(event)" title="Đăng xuất">
                                    <i class="fas fa-sign-out-alt"></i>
                                </a>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- 3. XỬ LÝ ĐĂNG NHẬP THÔNG MINH (CLIENT SIDE) ---
        document.getElementById('modalLoginForm').addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(e.target);
            const inputVal = formData.get('ten_dang_nhap'); // Lấy giá trị người dùng nhập

            // --- BƯỚC 1: KIỂM TRA ĐỊNH DẠNG EMAIL ---
            const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            let action = 'login'; // Mặc định là Admin/Nhân viên

            if (emailPattern.test(inputVal)) {
                // -> NẾU LÀ EMAIL: Chuyển sang API Khách hàng
                action = 'shopLogin';

                // (Tùy chọn) Gán thêm biến chuẩn cho Controller khách hàng dễ đọc
                formData.append('email', inputVal);
                formData.append('password', formData.get('mat_khau'));
            }
            ...
            try {
                // Gọi đúng action đã xác định ở trên
                ...
                if (data.success) {
                    ...
                    setTimeout(() => {
                        // Nếu API trả về link redirect (ví dụ vào admin) thì đi theo, không thì reload trang chủ
